...
/* GENERATOR SETTINGS
------------------------------------------------------- */
$settings['generator_name'] = $lang['generator_movie_name'];
$settings['color_sheme']    = 'movie';
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $input['crack']         = (!empty($_POST['crack']) ? clean_input($_POST['crack']) : '');
    ...
    $input['nfo']           = (!empty($_POST['nfo']) ? clean_input($_POST['nfo']) : '');
    ...
    $input['vstream']       = (!empty($_POST['vstream']) ? clean_input($_POST['vstream']) : '');
    $input['runtime']       = (!empty($_POST['lifetime']) ? clean_input($_POST['lifetime']) : '');
    $input['astream']       = (!empty($_POST['astream']) ? clean_input($_POST['astream']) : '');
...
------------------------------------------------------- */
    ...
    $validation = $site_template->parse_template("validation_movie");
    ...
    $site_template->register_vars(array(
        ...
        "wrapper"                     => $settings['wrapper_size'],
        "head_icon"                   => $settings['head_icon'],
        "website_title"               => $lang['website_title_movie'],
    ...
    $site_template->print_template($site_template->parse_template("movie"));
}
