...
/* GENERATOR SETTINGS
------------------------------------------------------- */
$settings['generator_name'] = $lang['generator_software_name'];
$settings['color_sheme']    = 'software';
$settings['wrapper_size']   = 'wrap-2';
$settings['head_icon']      = 'fa-dot-circle-o';
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $input['language']      = (!empty($_POST['language']) ? clean_input($_POST['language']) : '');
    ...
    $input['build']         = (!empty($_POST['build']) ? clean_input($_POST['build']) : '');
    $input['version']       = (!empty($_POST['version']) ? clean_input($_POST['version']) : '');
    $input['language']      = (!empty($_POST['language']) ? clean_input($_POST['language']) : '');
    ...
    ------------------------------------------------------- */
    $tpl_bbcode = bbCode($input, $upl_tpl, $GLOBALS['bbCodeSoftware']);
...
------------------------------------------------------- */
    ...
    $site_template->register_vars(array(
        ...
        "wrapper"                     => $settings['wrapper_size'],
        "head_icon"                   => $settings['head_icon'],
        "website_title"               => $lang['website_title_software'],
        ...
        "lang_donate"                 => $lang['donate'],
        "lang_version"                => $lang['version'],
        "lang_build"                  => $lang['build'],
        "lang_build_tip"              => $lang['build_tip'],
        "lang_format"                 => $lang['format'],
        "lang_format_tip"             => str_replace('{format}', 'exe, img, iso, ...', $lang['format_tip']),
    ...
    $site_template->print_template($site_template->parse_template("software"));
}
